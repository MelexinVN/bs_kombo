/*
* spi.c
* Ѕиблиотека интерфейса SPI
* ћикроконтроллеры: ATmega8/48/88/168/328
*										STM32 (необходиом указать номер интерфейса)
*/

#include "spi.h"				//добавл€ем заголовочный файл
...
void spi_init(void)
{
	...
	#ifdef STM32_LL
	LL_SPI_Enable(SPI1);										//включаем spi, основна€ инициализаци€ генерируетс€ автоматически
	#endif
}
...
uint8_t spi_change_byte(uint8_t byte)
{
	...
	#ifdef STM32_LL
	uint8_t dt = 0;
	while(!LL_SPI_IsActiveFlag_TXE(SPI1)) {}	//ждем пока не подниметс€ флаг txe (готовность к передаче)
